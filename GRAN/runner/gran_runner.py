...
class GranRunner(object):
  ...
  def test(self):
    ...
    if self.config.test.is_test_ER:
      ...
      base_path = os.path.join(self.config.dataset.data_path, 'save_split')
      if not os.path.exists(base_path):
        os.makedirs(base_path)
      save_graph_list(graphs_gen, os.path.join(base_path, '{}_dev.p'.format(self.config.dataset.name)))
      dev_graphs = load_graph_list('data/save_split/DD_dev.p')
      print(len(dev_graphs))
